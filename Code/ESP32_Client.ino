...
while (millis() - startTime < 1000) { // max. 1 Sekunde warten
  ...
  // --- sauberer Disconnect ---
  if (client.connected()) client.disconnect(); // sendet MQTT DISCONNECT
  delay(50);
  net.stop();                   // TCP-Socket zu
  delay(50);

  // WLAN trennen (Credentials NICHT löschen)
  if (WiFi.status() == WL_CONNECTED) {
    WiFi.disconnect(false /*wifioff*/, true /*eraseARP*/);
    delay(50);
  }
